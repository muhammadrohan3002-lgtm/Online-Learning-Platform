  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        toast.error(
          res.error.message ||
            "Failed to sign in with Google. Please ensure your dev server is restarted after editing .env"
        );
      ...
      toast.error(
        err?.message ||
          "Failed to connect to Google. Please check your network and server configuration."
      );
      setGoogleLoading(false);
    }
  };
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-[calc(100vh-140px)] flex items-center justify-center"><span className="loading loading-spinner loading-lg text-primary"></span></div>}>
      <LoginForm />
    </Suspense>
  );
}